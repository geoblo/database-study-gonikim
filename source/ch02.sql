...
VALUES (1, '빅맨', '2장 패티의 대표 클래식 버거입니다.', 5300, 223, 583, 27);
...
INSERT INTO burgers
VALUES (2, '빅맨2', '2장 패티의 대표 클래식 버거입니다.', 5300, 223, 583, 27);
...
SELECT name, price -- 특정 컬럼만 조회: 이름, 가격
FROM burgers; -- 버거 테이블에서
...
SET price = 500
WHERE name = '빅맨'; 
-- 에러 발생 이유? 안전모드에서는 KEY(기본키나 유니크키) 컬럼이 아닌 다른 값을 사용하면 막음
...
-- Quiz: '슈비두밥 버거'가 단종됐다면, 이를 위한 데이터 삭제 쿼리는?
DELETE FROM burgers
WHERE name = '슈비두밥 버거'; -- 에러 발생: 수정과 동일하게 조건은 반드시 키값을 사용
