...
CREATE TABLE users (
	id INTEGER, -- 기본키
	email VARCHAR(100) NOT NULL UNIQUE, -- 대체키: 데이터베이스에서 후보키 중 기본키로 선택되지 않은 키
	name VARCHAR (50),
	PRIMARY KEY (id)
);
CREATE TABLE orders (
...
-- 예: 주문은 반드시 사용자가 있어야 한다 -> 필수
-- user_id INT NOT NULL -- 필수
...
-- 주문은 특정 사용자를 참조할 수도 있고, 또는 참조하지 않을 수도 있다. 즉, 비회원인 경우에도 주문을 할 수 있다.(0~1)
...
-- ③ __________: 엔티티의 특성이나 정보를 나타내는 항목으로 데이터베이스로 구현했을 때 컬럼이 됨
...
-- 3) 고유키 및 인덱스 등 다양한 제약 조건 지정(실습: ERD 만들기)
-- users 테이블의 email과 products 테이블의 name을 고유키로 지정
-- 고유키는 UK로 표시
-- products 테이블의 name을 인덱스 처리(참고: UNIQUE 지정 시 자동으로 인덱스 생성됨)
...
-- 풀 테이블 스캔(Full Table Scan: 테이블 전체를 첫 번째 행부터 마지막 행까지 하나씩 차례대로 비교) 하지 않고도 
-- 데이터를 효율적으로 찾을 수 있도록 해 줌
...
-- 형식:
-- 중복 허용 인덱스
CREATE INDEX idx_users_email -- 인덱스 이름
ON users (email); -- 테이블 (인덱스 컬럼)

-- 인덱스의 동작 원리(간단히 소개)
-- 특정 컬럼을 정렬해서 저장하고, 이를 통해 원본 데이터에 빠르게 접근할 수 있는 특별한 목차
-- 책 맨 뒤 인덱스(찾아보기)를 보면
-- 1) 책의 중요한 키워드들이 알파벳순, 가나다순(정렬된 순서)으로 정리되어 있음
-- 2) 각 키워드 옆에는 그 키워드가 등장하는 페이지 번호가 적혀 있음
-- 데이터베이스의 인덱스도 책의 인덱스(찾아보기)와 정확히 동일한 역할
-- 인덱스는 특정 컬럼(들)의 데이터를 기반으로 생성되는, 원본 테이블과는 별개의 특수한 자료 구조
-- 1) 인덱스 내부의 데이터는 항상 정렬된 상태를 유지
-- 2) 인덱스는 지정된 컬럼(예: 상품명)의 값과, 해당 값을 가진 실제 데이터 행의 위치(예: 주소값, PK 값 등)를 한 쌍으로 저장


-- 4. DB 구현하기(실습: ERD로 데이터베이스 구현하기)
...
CREATE TABLE orders (
	...
	user_id INTEGER NOT NULL, 			-- 사용자 아이디(필수 입력) <- 필수적 관계
	PRIMARY KEY (id), 					-- 기본키 지정: id
...
CREATE TABLE payments (
	...
	order_id INTEGER NOT NULL UNIQUE, 	-- 주문 아이디(필수 입력, 고유키 지정) <-- 필수적 관계
	PRIMARY KEY (id), 					-- 기본키 지정: id
...
CREATE TABLE order_details (
    ...
    -- order_id, product_id 한 쌍을 이루어서 고유한 값을 가져야만 한다.
    -- 의미: 하나의 주문에 같은 상품이 중복으로 담기지 않게 하려고 설정(같은 상품을 여러 개 담을 경우 count로 관리)
);
...
-- ② __________: 컬럼이 NULL 값을 가질 수 없게 하는 제약 조건
-- ③ __________: 컬럼에 입력되는 값이 특정 조건을 만족하도록 하는 제약 조건
-- ④ __________: 컬럼에 값이 입력되지 않을 때 기본으로 저장된 값을 설정하는 제약 조건
-- ⑤ __________: 데이터베이스에서 데이터의 검색 및 정렬 성능을 빠르게 해주는 데이터 구조로 테이블의 특정 컬럼에 생성해 활용
...
-- 위반 내용
-- 반복되는 컬럼 구조: 상품1, 상품2, 상품3처럼 동일한 의미(상품명)를 가지는 속성이 여러 개의 컬럼으로 반복되고 있음
-- 다중값 저장: 상품1 컬럼에 "가방,후드"라는 여러 개의 값을 하나의 칸에 넣고 있음
...
-- 용어 정리
-- 이행적 함수 종속: 기본키가 아닌 일반 속성이 다른 일반 속성을 결정하는 것
...
-- ② __________: 테이블의 컬럼을 더 이상 분해할 수 없도록 나누는 것
