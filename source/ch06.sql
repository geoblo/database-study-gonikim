...
-- FK를 걸 수 있는 대상이 여러 개라면, 기본 선택지는 항상 PK여야 한다.
...
-- 예: 고객 테이블에 customer_id(PK)가 1~3까지 저장되어 있는데 주문 테이블에 customer_id(FK) 값이 99가 입력된다면?
...
CREATE TABLE settings (
    ...
    user_id INT UNIQUE, -- 1:1 연결을 위해 UNIQUE
    PRIMARY KEY (id),
    FOREIGN KEY (user_id) REFERENCES users(id)
);
