...
FROM burgers
GROUP BY category; -- 카테고리 별로 그룹을 묶고, 각 그룹에서 가격의 최대값/최소값을 구함
...
CREATE TABLE transactions (
	id INT, -- 아이디
    amount DECIMAL(12, 2), -- 거래금액(12자릿수: 정수 10자리까지, 소수점 이하는 2자리까지 허용)
    msg VARCHAR(15), -- 거래처
...
-- Quiz: 구글과 거래한 금액의 총합은?
SELECT SUM(amount)
FROM transactions
WHERE msg = 'Google';

-- Quiz: 페이팔과 거래한 금액의 최대값/최소값은?
SELECT MAX(amount), MIN(amount)
FROM transactions
WHERE msg = 'PayPal';

-- Quiz: 쿠팡 및 아마존과 거래한 총 횟수는?
SELECT COUNT(*)
FROM transactions
