...
-- 문제 1
-- 다음 쿼리에서 id 컬럼의 자료형으로 SMALLINT가 적합한 이유를 설명하세요.
...
-- 실제 저장된 문자열의 길이에 따라 메모리를 사용해 공간 낭비를 줄인다.
-- CHAR(50): 최대 50자의 고정 길이 문자열을 저장하며, 
-- 데이터 길이와 상관없이 항상 50자 공간을 사용한다.
...
-- 정답: 이미지, 오디오, 동영상 같은 크기가 큰 파일을 저장하는 데 적합하다.
...
-- 정답:
-- FLOAT와 DOUBLE은 둘 다 부동 소수점 방식의 실수를 저장한다.
-- 다만 차지하는 메모리 크기가 달라 FLOAT는 4바이트, DOUBLE은 8바이트를 차지한다.
-- 따라서 DOUBLE은 FLOAT보다 넓은 범위의 실수를 저장해 실수를 좀 더 정밀하게 표현할 수 있다.
...
-- 정답: LIKE 연산자에서 _는 정확히 임의의 한 글자를 대체한다.
...
-- 정답: 15 (EXTRACT(DAY FROM date) 함수는 날짜에서 일(Day)을 추출합니다.)
...
-- 정답: 15 (HOUR() 함수는 시간에서 시간(Hour)을 추출합니다.)
...
-- 정답: 주문 시간이 9시부터 17시까지인 주문을 조회한다.
...
-- DATE 또는 DATETIME 타입으로 암시적 변환되어 비교됨
...
-- MySQL은 기본적으로 문자열을 DATE로 변환할 때, 포맷이 약간 틀려도 자동 보정해줌
-- 항상 두 자리 형식 (YYYY-MM-DD)을 명시적으로 사용하는 것이 좋음
