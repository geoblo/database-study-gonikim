...
-- 정답: 2
-- 1번은 제약 조건(UNIQUE, FK), 정규화의 효과
-- 3번은 인덱스, 쿼리 튜닝 등 물리적 설계
...
-- ① 회원, 도서, 대출 기록 등의 개체(엔티티)와 개체를 설명하기 위한 관계를 정의한다.
-- ② 도서 테이블의 재고 컬럼에 UNSIGNED 제약 조건을 추가한다.
-- ③ 대출 기록 테이블의 외래키 관계를 명시한다.
-- ④ 테이블의 인덱스 설계를 통해 쿼리 성능을 최적화한다.

-- 정답: 1
-- 2번 제약 조건 추가는 물리적 모델링 단계에서 수행
-- 3번 외래키 설정은 논리적 모델링 단계에서 다룸
-- 4번 인덱스 설계는 물리적 모델링 단계에서 수행
...
-- ① 대출 기록 테이블에서 borrow_date 컬럼에 기본값으로 현재 날짜를 설정한다.
-- ② 회원과 대출 기록 간의 관계를 일대다로 정의한다.
-- ③ 회원 테이블과 대출 기록 테이블 간의 ER 다이어그램을 작성한다.
-- ④ 대출 기록 테이블에서 borrow_date 컬럼을 생성한다.

-- 정답: 1
-- 2번 개체 간의 관계(일대다 관계) 정의는 논리적 모델링에서 수행
-- 3번 대출 기록 테이블이 중간 테이블이므로 논리적 모델링에 해당
-- 4번 컬럼을 정의하는 것은 논리적 모델링에서 수행
