...
-- 정답: 2
-- "HAVING이 SELECT보다 먼저인데 별칭을 어떻게 알아?"라고 생각할 수 있습니다.
-- 하지만!
-- 대부분의 DBMS는 실제로 SELECT의 별칭을 HAVING에서 예외적으로 허용
-- 즉, 논리적 실행 순서와는 별개로, HAVING은 SELECT의 별칭을 인식할 수 있는 유일한 "앞쪽 절"
-- 표준 SQL은 식 자체 사용을 권장
...
-- (정리) SELECT 별칭(alias)의 사용 가능 여부
-- WHERE: 절대 불가(WHERE는 SELECT보다 먼저 실행되므로 별칭을 아직 모름)
-- GROUP BY: MySQL은 편의상 허용하지만 표준은 아님, 표준 SQL은 식 자체 사용을 권장
-- HAVING: 예외적으로 널리 허용(논리적 실행 순서와는 별개로 SELECT의 별칭을 인식), 표준 SQL은 식 자체 사용을 권장
